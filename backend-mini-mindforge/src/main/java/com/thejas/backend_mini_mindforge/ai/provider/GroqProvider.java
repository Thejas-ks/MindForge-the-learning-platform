...
public class GroqProvider implements AIProvider {
    ...
    @PostConstruct
    public void init() {
        log.info("=== GroqProvider initialized ===");
        log.info("Groq key: " + (apiKey != null && !apiKey.isBlank()
    ...
    private String call(String prompt, List<Map<String, String>> history) {
        try {
            ...
            log.info("[Groq] Status: " + res.getStatusCode());
            ...
            log.severe("[Groq] HTTP " + e.getStatusCode() + ": " + e.getResponseBodyAsString());
            ...
            log.severe("[Groq] Timeout: " + e.getMessage());
            throw new RuntimeException("AI service timed out. Please try again.");
        } catch (RuntimeException e) {
            throw e;
        } catch (Exception e) {
            log.severe("[Groq] Error: " + e.getClass().getSimpleName() + " - " + e.getMessage());
